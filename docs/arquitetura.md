...
- **`busca-voos`** — autenticada (valida JWT). Faz fan-out por programa para a API IN8
  (`http://apiv2.buscamilhas.com`), calcula o preço em R$ lendo `mileage_rates` e normaliza o retorno
  (`{ sucesso, origem_dados, alertas, resultados: { ida, volta } }`). Fallback de demonstração via
  secret `BUSCAMILHAS_MOCK`. Ver [`api-busca-milhas-in8.md`](api-busca-milhas-in8.md).
